...
def clean_num(v):
    ...
    try:
        ...
        if not s or s.lower() == 'n/d': return 0.0
...
def run():
    ...
    try:
        ...
        # Mappa invasi con nomi puliti e maiuscoli
        invasi_map = {str(d['diga']).strip().upper(): d for d in invasi if isinstance(d, dict)}
        ...
        for s in ana:
            ...
            if g_data["exists"]:
                output["stazioni"].append({
                    "nome": s.get('stazione'), "source": "GOOGLE", "lat": lat + 0.005, "lon": lon + 0.005,
